...
namespace SV.Maat.Micropub
{
    ...
    public class MicropubController : ControllerBase
    {
        ...
        public IActionResult Create([FromBody]MicropubJsonCreateModel post)
        {
        ...
        private IEnumerable<Entry.MediaLink> ParseMediaReference(IEnumerable<dynamic> items, string type)
        {
            ...
            {
                if (item is string || item.ValueKind == JsonValueKind.String)
                {
                    media.Add(new Entry.MediaLink { Url = item.ToString(), Type = type });
                }
                else
                {
                    var itemAsJsonEl = (JsonElement)item;
                    var m = new Entry.MediaLink { Url = itemAsJsonEl.GetProperty("value").ToString(), Type = type };
                    try
                    {
        ...
        private BaseCommand<Entry> GetRemoveCommand(UpdateModel post, Entry entry)
        {
        ...
        private BaseCommand<Entry> GetReplaceCommand(UpdateModel post, Entry entry)
        {
        ...
        private BaseCommand<Entry> GetAddCommand(UpdateModel post, Entry entry)
        {
